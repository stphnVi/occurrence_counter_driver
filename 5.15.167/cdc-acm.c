 ...
 */
static ssize_t bmCapabilities_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t wCountryCodes_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev);
	struct acm *acm = usb_get_intfdata(intf);
...
static ssize_t iCountryCodeRelDate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev);
	struct acm *acm = usb_get_intfdata(intf);
...
static int get_serial_info(struct tty_struct *tty, struct serial_struct *ss)
{
	...
	ss->closing_wait = acm->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : jiffies_to_msecs(acm->port.closing_wait) / 10;
	return 0;
}
...
static int set_serial_info(struct tty_struct *tty, struct serial_struct *ss)
{
	...
	closing_wait = ss->closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : msecs_to_jiffies(ss->closing_wait * 10);
...
static void acm_disconnect(struct usb_interface *intf)
{
	...
	if (!acm->combined_interfaces)
		usb_driver_release_interface(&acm_driver, intf == acm->control ? acm->data : acm->control);
...
static const struct usb_device_id acm_ids[] = {
	{USB_DEVICE(0x0403, 0x6001), /* Raspberry Pi*/
...
static int __init acm_init(void)
{
	...
	acm_tty_driver->driver_name = "cdc-acm",
	acm_tty_driver->name = "ttyRaspberryPi",
